...
# Excercise 2
## Loading data a)
data_features = readRDS('top_features.rds')
data_train = readRDS('train.rds')
data_test = readRDS('test.rds')
data_features

# b)
#Task
task = makeClassifTask(data = data_train, target = ""  id = "CV")
task2 = makeClassifTask(data = data_train, target = "", id = "Bootstrap")
...
# Train
mod_svm = train(learner = lrn_cl_svm, task = task)

# PREDICT e)
pred = predict(object = mod_svm, newdata = data=data_test)
pred

# Performance f)
performance(pred = pred, measures = list(mmce, acc)) #mmce not needed for this task
...
# DBSCAN
data_s_d = dist(data_s, method = 'euclidean')
kNNdistplot(data_s, k=4)
abline(h=1.6, col='red', lty="dashed")
dbscan_cl = dbscan(data_s_d, eps = 1.6, minPts = 5)
dbscan_cl$cluster
...
data$dbscan = dbscan_cl$cluster

pairs(data[,1:2], col=data$dbscan+3, pch='X', cex=1.5)
